...
#从mysql通过hive插入到库会非常慢，1条数据和5000条件的数据的时间是一样的，应该使用dataX导入，
...
def fetch_data_from_mysql(bizdate):
    ...
    try:
        with connection.cursor() as cursor:
            sql = f"SELECT {','.join(columns)} FROM customer WHERE {where_clause}"
...
def insert_into_hive_partitioned(data, bizdate):
    ...
    # overwrite  将之前的数据覆盖
    insert_sql = f"""
    INSERT overwrite TABLE ods_customer PARTITION(dt='{bizdate}')
    VALUES {values_str}
    """
    # 记录代码块执行之前的时间
    start_time = time.time()
    try:
        print("Data inserted start.")
        ...
        print(f"Execution time: {execution_time} seconds,data_length: {data_length}")
...
if __name__ == "__main__":
    bizdate = '2025-01-14'  # Example date, replace with actual business date
